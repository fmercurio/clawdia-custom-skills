...
- optional `proposal_staging_path`, an existing owner-only directory strictly below the instance root; it is opened through descriptor-pinned no-follow traversal and re-bound by inode identity after publication, so a symlink or identity change rejects and cleans the proposal artifact. Before a proposal is written, the tenant policy must allow its fixed `engineering` / `internal` / `low` envelope. When absent, the server exposes only its four read-only retrieval tools

Artifacts are owner-only (`0o600`) and deterministic. `run_mcp.py` rejects a symlinked `runtime-config.json` file, then canonicalizes the accepted config path before deriving the descriptor-pinned instance root; relative invocation and platform path aliases therefore resolve to the physical absolute root without weakening no-follow staging traversal.
