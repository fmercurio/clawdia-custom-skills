...
- optional `proposal_staging_path`, an existing owner-only directory strictly below the instance root; it is opened through descriptor-pinned no-follow traversal and re-bound by inode identity after publication, so a symlink or identity change rejects and cleans the proposal artifact. When absent, the server exposes only its four read-only retrieval tools
