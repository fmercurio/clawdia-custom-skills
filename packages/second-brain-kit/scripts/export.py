...
def export(root: Path, output: Path) -> None:
    ...
    all_files=sorted([p for p in root.rglob("*") if p.is_file() and p.suffix not in {".pyc",".zip"} and "__pycache__" not in p.parts and ".brain-index" not in p.parts],key=lambda p:p.relative_to(root).as_posix())
...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--output",required=True); a=p.parse_args(); root=Path(__file__).resolve().parent.parent; output=Path(a.output).expanduser().resolve(); export(root,output); print(output); return 0
