...
from kitlib import LAYERS, config_path, hermes_home, load_config, safe_slug
...
def main() -> int:
    ...
    a=p.parse_args(); home=hermes_home(a.hermes_home); cfg=load_config(config_path(home,a.profile)); vault=Path(cfg["vault_path"]); script=search_script()
    if a.action=="push":
        folder=vault/LAYERS[a.layer]; folder.mkdir(parents=True,exist_ok=True); target=folder/f"{safe_slug(a.title)}.md"; today=date.today().isoformat()
        if target.exists(): print(json.dumps({"ok":False,"error":"note already exists","path":str(target)})); return 2
